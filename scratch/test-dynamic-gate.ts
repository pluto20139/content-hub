import { parseXUrl, getDeepLink, PLATFORMS } from "../packages/shared/src/index";
import { XAdapter } from "../scripts/cron/src/adapters/x";
...
assert(res1 !== null && res1.handle === "elonmusk", "解析标准 X 主页链接 (https://x.com/elonmusk)");

const res2 = parseXUrl("https://twitter.com/OpenAI?s=20");
assert(res2 !== null && res2.handle === "OpenAI", "解析 Twitter 带 Query 链接 (https://twitter.com/OpenAI?s=20)");

const res3 = parseXUrl("https://mobile.x.com/satyanadella");
assert(res3 !== null && res3.handle === "satyanadella", "解析移动端域名链接 (https://mobile.x.com/satyanadella)");

const res4 = parseXUrl("@sama");
assert(res4 !== null && res4.handle === "sama", "解析 @handle 账号 (@sama)");
