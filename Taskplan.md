...
# RSSHub
RSSHUB_URL=                # RSSHub 实例地址（GitHub Secrets + Supabase Secrets 双配置）
RSSHUB_API_KEY=            # RSSHub 鉴权密钥（同上）
...
**动作**：创建 `scripts/cron/src/lib/throttle.ts`，实现 `sleep(ms: number): Promise<void>` 和 `withPlatformThrottle(platform: string, fn: () => Promise<T>): Promise<T>` 封装。B站和知乎同平台请求间强制 sleep ≥ 1500ms（SPEC 要求），YouTube 无此限制。
...
### Step 6.9 — ZhihuAdapter — fetchLatest
...
>
> **注意**：`cookie_status` 持久化仅对 B站有意义（用户可扫码刷新），YouTube/知乎短路时不持久化状态，仅在 Cron 日志中记录"平台级故障，跳过整组"。

**自测标准**：
- [ ] 完整流程执行成功，CronResult 各字段正确
- [ ] 三个平台组并行执行（时间 ≈ max(B站时间, YouTube时间, 知乎时间)）
...
**动作**：创建 `.github/workflows/cron-fetch.yml`，配置每 30 分钟触发（SPEC 配置），执行 `scripts/cron` 中的脚本。配置所需 Secrets（SUPABASE_URL、SUPABASE_SERVICE_ROLE_KEY、YOUTUBE_API_KEY、RSSHUB_URL、RSSHUB_API_KEY、WECOM_WEBHOOK_URL）。
...
- [ ] `parse-url` Edge Function 可读取 `RSSHUB_URL` + `RSSHUB_API_KEY` 环境变量（解析知乎 URL 获取昵称时需要，通过 `supabase secrets set RSSHUB_URL=xxx RSSHUB_API_KEY=xxx` 配置）
...
**动作**：实现顶部平台筛选 Tab（SPEC 9.3）：全部 / B站 / 知乎 / YouTube。Tab 配置定义在 `apps/h5/src/constants/tabs.ts`。
...
- [ ] `last_content_at` 距今 95 天 → 显示"⚪ 该博主已超过 95 天未更新"（红色，SPEC 9.5）
...
### Step 11.2 — 端到端流程：三平台覆盖

**动作**：分别添加 B站/YouTube/知乎博主各一个，验证 Cron 三个适配器均能正常抓取。
...
- Phase 6 的三个适配器（B站/YouTube/知乎）可并行
